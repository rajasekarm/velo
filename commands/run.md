---
description: Velo — Run. Executes a plan that Plan has frozen — milestone-by-milestone delegated work, green checks plus one independent review per milestone, and local commits on the plan file's milestone branches. Writes only progress marks into the plan file; never pushes, merges, or opens PRs, and calls Verify only to collect acceptance evidence.
argument-hint: Name the approved plan to run — a slug, the work's name, or the plan file path; Run executes it milestone by milestone
---

# Velo — Run

Run is Velo's delivery mode: it takes a plan that `/velo:plan` has frozen and turns it into shipped, locally committed work, one milestone at a time. Run resolves the plan file at `.velo/tasks/<slug>/task-breakdown.md`, executes each milestone's task lines through delegated builders, gates the milestone on green checks plus one independent adversarial review, and commits the milestone's work locally on the branch the plan file names. The plan is the contract; Run adds progress marks and commits, never opinions.

This build of Velo ships Plan, Run, and Verify. Run executes exactly one plan per invocation — the one the user names or the exact approved plan file passed by the Plan workflow.
...
**Consumes only frozen plans.** Run executes only a plan file `/velo:plan` owns and has frozen: `Plan-version:` present, `Approval:` reading `v<N> · approved by <approver> · <YYYY-MM-DD HH:MM>`, and `Phase:` reading `PLAN (Plan — v<N> approved)` — or a plan file this mode has already been executing, resumed per **Pauses, tripwires, and resume** below. Anything else is refused with a plain explanation, never worked around.

**The plan body is read-only.** The original request, the assumptions, the architecture, acceptance criteria and verification expectations, the constraints text, the milestones, and the text of every task line are Plan's writing, frozen at approval. Run never rewrites them. Run writes ONLY progress marks: task-line `Status:` values, `Phase:`, `Last gate passed:`, `Rework cycles:`, `Updated:`, event bullets in `## Constraints/notes`, and the task's row in `.velo/tasks/index.md` — exact values per **Progress marks** below. Nothing else in the plan file, and nothing else under `.velo/`, changes in Run.

**Repository scope, exactly.** Run creates each milestone's branch — the name the plan file pins in that milestone's `Branch:` line — and commits that milestone's work locally with evidence in the message. M1's branch cuts from the repository's default branch; M<i>'s branch cuts from the previous milestone's tip when that work is not yet on the default branch, else from the default branch. That is the entire git surface. Run never pushes, never merges, never opens a PR, and never touches origin in any way — shipping past the local commit is the maintainer's explicit call, made outside Run.

**Fail-closed pauses.** A material change surfaced mid-run, a required check that stays red after in-scope rework, a plan file gone missing, unreadable, or edited out from under the run, or a conflicting dirty working tree — each pauses the run per **Pauses, tripwires, and resume**: mark, record, tell the user, stop. Run never widens scope silently, never improvises past a blocker, and never resumes a material-change pause without a newly frozen plan version.

**Mode boundaries.** Run may call Verify for acceptance evidence and then resume this playbook. It never starts Plan or any other mode automatically. Run never plans: a gap in the plan routes back to `/velo:plan`, never gets filled in on the fly.

---

## Step 1 — Resolve the plan

The Plan workflow may enter Run after explicit approval and a verified freeze, passing the exact plan file path and approved version. This handoff uses every check below; it bypasses no freeze or pause rule.

The input names the plan to run: a task slug, the work's name, or a path to the plan file or its folder. Resolve it to exactly one plan file at `.velo/tasks/<slug>/task-breakdown.md`.
...
- **No match, or the plan file is missing or unreadable** → say so plainly and stop. Do not hunt for a near-match to run instead.

## Step 2 — Verify the freeze

A plan file qualifies on first entry only when all three hold:
...
- **Not a plan file** — no `Plan-version:` key, or a shape Run does not recognize → refuse; Run consumes only plan files in the plan format Plan writes.

A plan file already carrying this mode's own marks — `Phase:` reading `RUN` — re-enters per the resume rules in **Pauses, tripwires, and resume**; recorded progress is picked up, never redone.
...
2. **Create the branch.** Create the milestone branch the plan file names in `M<i>`'s `Branch:` line (`<slug>-m<i>`) and switch to it. M1's branch cuts from the repository's default branch; M<i>'s branch cuts from the previous milestone's tip when that work is not yet on the default branch, else from the default branch. A branch already carrying that name that this run has no record of creating is a tripwire — pause and name it; never adopt foreign history (a stacked base — a tip the plan file's ship bullet records — is expected history, not foreign). On a resume that the plan file corroborates, switch back to the existing milestone branch instead of creating a second one.
3. **Mark the open** in one plan file write: `Phase: RUN (Run — M<i> building)`, `Updated:` from the local clock, and — when the index row still reads `planned` — the row flipped to `in-progress`.
...
**Acceptance evidence is part of (a).** When the approved plan defines acceptance criteria, verify every criterion owned by this milestone using its specified method and cases. Record actual results as Verification event bullets below. Every required criterion due for the milestone needs current passing evidence; `fail` enters rework and `blocked` pauses. Criteria belonging to later milestones do not gate earlier ones, but all must pass before final completion. Call [Verify](verify.md) with the exact plan path/version, milestone, due AC ids, target/setup, and current revision. On Codex, load its [skill](../.agents/skills/verify/SKILL.md); on Claude Code, follow the Verify playbook directly. Verify returns evidence without editing the plan; Run appends the result bullets and handles rework or pauses. Browser checks may use Chrome DevTools MCP or Playwriter, as selected in the plan or available on the host. Do not add verification requirements to older frozen plans that do not define them.

**(b) One independent adversarial review passes.** The milestone's changes are reviewed against the plan file — deliverables delivered, assumptions respected, constraints held, nothing beyond scope:
...
- **Single-flow host** → Run performs a structured adversarial self-review — deliverable by deliverable, constraint by constraint, actively hunting for what is missing or wrong — and records in the plan file that the review was NOT independent; the ship bullet carries the mark, per **Progress marks**.

**Findings drive rework.** A failed check or a review finding sends the work back to the responsible task line's builder, scoped to the finding: the line returns to `Status: in-flight`, the fix lands, the checks re-run, and `Rework cycles:` advances by one in the plan file header. Rework stays inside the milestone and inside the plan's scope. A required check that stays red after in-scope rework is a tripwire — pause; do not keep grinding, and do not widen scope to force green.
...
2. **Mark the ship** in one plan file write: `Last gate passed: MILESTONE_SHIP (Milestone ship — M<i>)`, the ship event bullet with the commit reference (format per **Progress marks**), and `Updated:`.
...
- **The last milestone shipped** → in one plan file write: `Phase: DONE (Done — delivered-and-committed on <slug>-m<final>)`, the index row flipped to `done`, and `Updated:`. Then announce and stop, for example:
...
## Progress marks — the only plan file writes
...
**`Updated:`** — `YYYY-MM-DD HH:MM`, local clock (`date`-sourced), advanced on every plan file write.

**Event bullets** — appended at the end of `## Constraints/notes`, in chronological order, one line each:

- Verification: `- M<i> verification · AC<n> · <pass|fail|blocked> · method <browser|api|command|manual> · provider <tool or human> · target <URL/route or check> · revision <commit + local-change description> · actions <cases performed> · expected <result> · observed <result or blocker> · evidence <observations or permitted reference> · <YYYY-MM-DD HH:MM>` — append a result per criterion/attempt; advance `Updated:`. A manual pass requires attributed user confirmation or supplied evidence. Verification results do not change the approved criterion text.
...
**The index row** — `planned` → `in-progress` when the first milestone opens, → `done` at the final ship; the flip to `in-progress` happens when a milestone opens and the row still reads `planned` — a resumed run leaves an `in-progress` row alone; the row's Updated date advances with every plan file write. A paused run stays `in-progress`.

Everything else — every header key not named above, the original request, the assumptions, the architecture, acceptance criteria and verification expectations, the constraints text, the milestones and task-line text, the `## Artifacts` section — is read-only in Run.
...
**The pause protocol**, one plan file write plus one message:
...
- a required check that stays red after in-scope rework, or a required acceptance criterion blocked by unavailable verification tools, access, environment, or test data
- the plan file missing, unreadable, or edited out from under the run mid-execution — and when the plan file itself cannot be written, tell the user and stop; the recorded marks land only where a plan file exists to hold them
...
**Resume.** Re-invoking `/velo:run` with the same plan file picks up from recorded state: the first unshipped milestone, its first task lines not yet `done`; `done` work is never redone. Two rules gate it:
...
- A **tripwire pause** resumes once the recorded blocker is gone — the tree clean, the plan file intact, the red check's cause fixed within scope. If clearing it changed the plan's deliverables, surface, risk class, or required evidence, that is a material change: the Plan route first, then resume.
