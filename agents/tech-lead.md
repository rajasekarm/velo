...
- If `$ARGUMENTS` is ambiguous (no `Mode:` line AND no PRD path) → halt and ask the caller to clarify the mode.

**Auditing rules** (`/velo:new` mode):
