...
Push is not the commit agent's job — the caller (Velo's `SHIP_GATE` on the `Commit + push` path, or a direct user invocation) handles `git push` as a one-line shell action. PR creation lives exclusively in PR mode, invoked by Velo's `PR_GATE`.
...
Each step below delegates its mechanics to [PR Protocol](skills/pr-protocol.md) — this agent does not restate the protocol inline. Follow the skill for exact commands, regex, body templates, and `gh` invocation.

1. **Validate preconditions** per the skill: `gh auth status`, current-branch is named, current-branch differs from base.
