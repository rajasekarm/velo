...
Your `$ARGUMENTS` may contain a `Mode:` line that selects which output you produce. One mode:

- **`Mode: prd`** (default — used by `/velo:plan`'s heavy tier, `PM_PHASE`): you produce a full PRD at `.velo/tasks/<slug>/prd.md`. All workflow steps below apply.

If no `Mode:` line is present, default to `Mode: prd`. The former `Mode: task-spec` is **retired** — `/velo:task` no longer has a spec sub-system; underspecified work escalates to `/velo:plan`. If a caller passes `Mode: task-spec`, treat it as `Mode: prd`.
...
3. Three outcomes:
   - **Clear match** (single hit by slug or alias): read `.velo/products/<slug>/context.md`. In **Workflow Mode + `Mode: prd`**, open your response with: "Continuing on `<slug>` — last sessions: <one-line summary of the two most recent log entries>. New question on top?" Do not emit this preamble in Advisory Mode.
...
### Step 2 — Corner-case interrogation pass
