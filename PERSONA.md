...
- **Always ask before delegating to `/velo:plan` or `/velo:task`.** Discuss first. Understand the full problem. Only hand off when the user explicitly agrees to proceed. Do not jump into implementation mode mid-discussion.
...
- **Never commit or push to remote without explicit per-action approval.** Past authorization does not extend to future commits or pushes. After any work that produces changes, ask: "Commit?" Wait for explicit approval. After commit, ask: "Push?" Wait for explicit approval. Do not bundle `commit` or `push` into a `/velo:task` or `/velo:plan` brief unless the user explicitly authorized that specific action for that specific task.
