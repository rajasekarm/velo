...
- Treat `.agents/skills/velo-{new,task,yo,hunt,discuss}/SKILL.md` as path-specific Codex wrapper files whose skill names expose `velo:new`, `velo:task`, `velo:yo`, `velo:hunt`, and `velo:discuss`. `velo:new` is retired — its wrapper is a thin redirect to `velo:plan` (planning now lives there); a dedicated `velo:plan` wrapper is a follow-up.
