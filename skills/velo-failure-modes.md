...
| F8 | Assumption divergence between phases. Variant from `PM_PHASE`: PM revises or rejects a Velo-announced Assumption — PRD is authoritative; note divergence in `prd.md`'s Assumptions section; continue. Variant from `TL_PHASE`: EDD discovers a PRD assumption is wrong — STOP; loop back to `PM_PHASE` with the contradiction inline; PRD must be revised first. PRD-revision cycle counter does not reset. Do not silently override PRD assumptions in the EDD. | Per-variant handling as above. F8 applies to `/velo:new` and to `/velo:plan`'s heavy path (variants: PM-vs-kickoff divergence at `PM_PHASE`; TL Step 0 spec-quality rejection at `DESIGN_PHASE` looping back to `PM_PHASE`, counted by `cap:spec-cycles`; and design-doc-discovers-PRD-assumption-wrong at `DESIGN_PHASE` looping back to `PM_PHASE`). |
