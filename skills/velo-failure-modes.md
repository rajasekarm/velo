...
| F7 | User asks Velo to write code | Decline per the Hard Rule. Use `ask-options`: `Route to <current-mode> agents` (when current mode is `task`/`plan`, restate the request inline as the brief), `Stay in current mode and rephrase`, `Abandon`. |
