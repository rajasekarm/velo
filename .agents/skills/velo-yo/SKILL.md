---
name: yo
description: Use when the user asks for /velo:yo, or brings Velo any request to triage — Velo routes it (build → plan/task, debug → hunt, review → review skills, discussion → discuss) or answers a settled question directly.
---

# Velo Yo

This is the Codex-discoverable wrapper for Velo's entry point — the front door that triages a request and routes it onward, or answers it in place. In this repo namespace, it should appear as `velo:yo`.

Velo workflow root: resolve by walking up from this `SKILL.md` to the directory containing `AGENTS.md`, `ADAPTER.md`, `TEAM.md`, and `commands/`. When this plugin is used from another repo, read Velo workflow assets from that root and treat the current working directory as the target project.

## Load Order

1. Load AGENTS.md first.
2. Read `ADAPTER.md` for runtime mappings.
3. Read `PERSONA.md` for Velo Engineering Manager behavior.
4. Read `commands/yo.md` for the workflow playbook.
5. Read only the skill files needed by the playbook.

## Codex Adaptation

- Treat this as a Codex wrapper around the existing Velo playbook.
- Do not treat this wrapper as an automatic Codex slash command.
- Resolve adapter concepts such as `ask-options`, `load-tool`, and `handoff-mode` through `ADAPTER.md`.
- Preserve yo mode as the entry point: triage and route; clarify, or answer conceptually — never do the work itself.
- Do not write code or edit artifacts in yo mode.
- If a Claude-only instruction cannot be mapped cleanly, state the mismatch and choose the closest Codex-native behavior.
